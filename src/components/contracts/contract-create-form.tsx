      ...
      <section className="mx-auto max-w-4xl overflow-hidden rounded-[1.5rem] border bg-[var(--surface)] shadow-2xl" aria-live="polite">
        ...
        <div className="p-6 md:p-8">
          <div className="grid gap-x-8 gap-y-5 rounded-2xl bg-[var(--surface-muted)] p-5 md:grid-cols-2">
            ...
            <Info label={tx(locale, "تاريخ ووقت الإنشاء", "Created at")} value={formatDateTime(success.createdAt, locale)} />
          </div>
...
function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) { return <label><span className="label">{label}{required && <span className="ms-1 text-red-600">*</span>}</span>{children}</label>; }
