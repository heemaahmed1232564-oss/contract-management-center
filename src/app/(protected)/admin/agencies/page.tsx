...
import { localizedName, tx, type Locale } from "@/lib/i18n";
import { getLocale } from "@/lib/i18n-server";

export const metadata = { title: "إدارة الشركات | Company management" };
...
export default async function AgenciesPage() {
  ...
  return <div className="space-y-6"><div><p className="mb-1 text-sm font-bold text-[var(--primary)]">{tx(locale, "المصدر الأول للاختيار", "The first selection source")}</p><h1 className="text-2xl font-black">{tx(locale, "إدارة الشركات", "Company management")}</h1></div>
    ...
    <section className="space-y-3">{agencies.map(agency => <details className="card p-5" key={agency.id}><summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[var(--surface-muted)]"><Building2 className="size-4" /></span><div><p className="font-black">{localizedName(locale, agency)} <span className="text-xs text-[var(--muted)]" dir="ltr">({agency.code})</span></p><p className="mt-1 text-xs text-[var(--muted)]">{agency._count.templates} {tx(locale, "قالب", "templates")} · {agency._count.generatedContracts} {tx(locale, "عقد", "contracts")}</p></div></div><div className="flex items-center gap-2"><Badge tone={agency.isActive ? "success" : "danger"}>{agency.isActive ? tx(locale, "فعالة", "Active") : tx(locale, "معطلة", "Disabled")}</Badge>{agency.googleDriveFolderId && <DriveTestButton id={agency.googleDriveFolderId} type="folder" />}</div></summary><div className="mt-5 flex justify-end"><ConfirmActionButton action={archiveAgencyAction} id={agency.id} title={tx(locale, "حذف هذه الشركة؟", "Delete this company?")} description={tx(locale, "سيتم إيقافها وإخفاؤها من إنشاء العقود مع الاحتفاظ بالسجلات القديمة.", "It will be disabled and hidden from contract creation while historical records remain.")} successMessage={tx(locale, "تم حذف الشركة.", "Company deleted.")} /></div><AgencyForm agency={agency} locale={locale} /></details>)}</section>
  </div>;
}
