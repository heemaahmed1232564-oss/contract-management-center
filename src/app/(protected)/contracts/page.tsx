...
export const metadata = { title: "سجل العقود" };
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="card grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-7">
        ...
        <select className="field" name="agency" defaultValue={params.agency}><option value="">{tx(locale, "كل الشركات", "All companies")}</option>{agencies.map(a => <option key={a.id} value={a.id}>{a.nameAr || a.name}</option>)}</select>
      ...
      <section className="card overflow-hidden">
        ...
        <div className="table-wrap"><table className="data-table"><thead><tr><th>{tx(locale, "المرجع", "Reference")}</th><th>{tx(locale, "الشركة", "Company")}</th><th>{tx(locale, "الباقة", "Package")}</th><th>{tx(locale, "العميل", "Client")}</th><th>{tx(locale, "الموظف", "Employee")}</th><th>{tx(locale, "القالب", "Template")}</th><th>{tx(locale, "القيمة", "Value")}</th><th>{tx(locale, "الحالة", "Status")}</th><th>{tx(locale, "الإنشاء", "Created")}</th><th>{tx(locale, "الإجراءات", "Actions")}</th></tr></thead><tbody>{contracts.map(c => <tr key={c.id}>
          <td className="font-bold" dir="ltr">{c.referenceNumber}</td><td>{c.agency.nameAr || c.agency.name}</td><td>{c.package.name}</td><td>{c.clientName || "—"}</td><td>{c.createdBy.name}</td>
          ...
          <td className="whitespace-nowrap">{formatMoney(c.price?.toString() ?? null, c.currency)}</td><td><div className="space-y-2"><ContractStatusBadge status={c.status} locale={locale} />{!['CREATING','FAILED','CERTIFIED'].includes(c.status) && <form action={updateContractStatusAction} className="flex gap-1"><input type="hidden" name="id" value={c.id} /><select name="status" defaultValue={c.status} className="max-w-28 rounded-lg border bg-[var(--surface)] p-1 text-[11px]"><option value="OPENED">Opened</option><option value="COMPLETED">Completed</option><option value="PDF_EXPORTED">PDF Exported</option><option value="SENT">Sent</option><option value="CANCELLED">Cancelled</option><option value="ARCHIVED">Archived</option></select><button className="rounded-lg bg-[var(--surface-muted)] px-2 text-[11px] font-bold" type="submit">{tx(locale, "حفظ", "Save")}</button></form>}</div></td><td className="whitespace-nowrap text-xs text-[var(--muted)]">{formatDateTime(c.createdAt)}</td>
